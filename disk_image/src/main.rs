...
fn create_fat_filesystem(fat_path: &Path, efi_file: &Path) {
    ...
    let efi_size_rounded = ((efi_size + mb - 1) / mb + 1) * mb;

    // create a new filesystem image file at the given path and set its length
...
fn create_gpt_disk(disk_path: &Path, fat_image: &Path) {
    ...
    // create a protective MBR at LBA0 so that disk is not considered unpartitioned
    // or unformatted on BIOS systems
